# DeepLinkParser
Provides a way to create, from `IActivatedEventArgs` or `string`, a `Dictionary<string,string>`-inheriting object that provides an additional `.Root` property to pull the base path of the URI (eg: in `MainPage/Options?option1=value1`, `.Root` would be `MainPage/Options`.
...
Both of these are createable using either `.Create(IActivatedEventArgs)` or `.Create(string)` factory methods. Should you wish to create one in a different manner, the default constructor is `protected` so inheriting from either of these can provide extensibility.
